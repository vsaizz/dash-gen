...
def fetch_exoplanet_data():
    ...
    try:
        ...
        df["Habitability Indicator"] = np.where(
            (df["Orbital Distance (AU)"].astype(float) > 0.95) &
            (df["Orbital Distance (AU)"].astype(float) < 1.67) &
            (df["Equilibrium Temperature (K)"].astype(float) > 180) &
            (df["Equilibrium Temperature (K)"].astype(float) < 310),
            1, 0
        )
        # Convert numeric columns to float, handle missing data
        numeric_cols = [
            "Mass (Earth Masses)", "Radius (Earth Radii)", "Orbital Period (days)",
            "Habitability Indicator", "Orbital Distance (AU)", "Eccentricity", "Equilibrium Temperature (K)"
        ]
...
star_type_options = sorted(exoplanets_df["Star Type"].dropna().unique())
discovery_method_options = sorted(exoplanets_df["Discovery Method"].dropna().unique())
...
with col2:
    if not filtered_df.empty:
        ...
        most_common_category = categories_counts.idxmax()
